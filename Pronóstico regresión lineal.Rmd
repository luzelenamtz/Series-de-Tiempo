...
test <- BTC_USD_tidy %>% select(Volume) %>% filter_index("2019 Jan" ~ "2024 Jan")
tstng_prds <- 6
frcst_prds <- 6
```
...
```{r}
gdp_us = gdp_us %>% mutate(gdp_base = cumsum(Volume))
gdp_us
```
...
gdp_us = gdp_us %>% select(Date, Volume) %>% as_tsibble(index='Date', regular = TRUE)
gdp_us
```


```{r}
train_gdp <- gdp_us %>% select(gdp_base) %>% filter_index("2021 Jan" ~ "2024 Jan")
test_gdp <- gdp_us %>% select(gdp_base) %>% filter_index("2020 Jul" ~ "2024 Jan")
```
...
```{r}
train_gdp$lag1 = c(NA, train_gpd$BTC_USD_tidy[1:length(train_gpd$BTC_USD_tidy)-1])
train_gdp
